...
/**
 * "Shop These Materials" + "Request Service Quote" bar.
 * The shop link appears only when an admin has curated active products.
 */
export function ServiceMaterialsBar({
  ...
  return (
    <div className="mt-10 rounded-[4px] border border-ud-burgundy/20 bg-ud-burgundy/[0.04] p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
      <div className="flex-1">
        <p className="text-sm font-bold text-ud-dark">Need materials for {label}?</p>
        <p className="text-xs text-ud-dark/55 mt-0.5 leading-relaxed">
          {hasMaterials ? "View the materials selected for this service, or request a project-specific quote." : "Request a project-specific materials and labour quote."}
        </p>
      </div>
      <div className="flex flex-wrap gap-3 flex-shrink-0">
        {hasMaterials && (
          <Link href="#materials" className="inline-flex items-center gap-1.5 bg-ud-burgundy text-white text-xs font-bold px-4 py-2.5 rounded-[4px] hover:bg-ud-burgundy-hover transition-colors whitespace-nowrap">
            Shop These Materials <ArrowRight size={13} />
          </Link>
        )}
        <Link
