...
import { Phone, Mail, ShoppingBag, Truck, CreditCard } from "lucide-react";
...
const FAQS = [
  {
    icon: ShoppingBag,
    q: "How do I order a house plan?",
    a: "Browse the Shop, choose a plan, select whether you want a digital download or a printed copy, add it to your cart, and check out. You'll receive your plan or a tracking update by email.",
  },
  {
    icon: CreditCard,
    q: "What payment methods do you accept?",
    a: "We accept M-Pesa, card payments (via Flutterwave), and bank transfer. All payments are processed securely, and you'll get a receipt by email once your payment is confirmed.",
  },
  {
    icon: Truck,
    q: "How are plans and orders delivered?",
    a: "Digital plans are delivered instantly via a secure download link after payment. Printed plans are dispatched to your delivery address, and you can follow progress on the order tracking page.",
  },
];
        ...
        <div className="max-w-content mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-16">
            {FAQS.map((faq) => {
              const Icon = faq.icon;
              return (
                <div key={faq.q} className="bg-ud-light-gray rounded-[4px] p-7 border-t-[3px] border-ud-burgundy">
                  <Icon className="w-7 h-7 text-ud-burgundy mb-4" strokeWidth={1.5} />
                  <h3 className="text-base font-bold text-ud-dark mb-2">{faq.q}</h3>
                  <p className="text-sm text-ud-dark/60 font-light leading-relaxed">{faq.a}</p>
                </div>
              );
            })}
          </div>
                ...
                <ul className="space-y-3">
                  {EMAIL_DIRECTORY.map((item) => (
                    <li key={item.email} className="flex items-start gap-3">
                      <Mail size={15} className="text-ud-burgundy flex-shrink-0 mt-0.5" />
                      <div>
