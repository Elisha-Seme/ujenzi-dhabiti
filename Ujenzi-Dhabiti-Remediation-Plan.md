...
- Preserve the current product architecture and existing data. No production deletes, resets, credential rotation, or DNS changes. A user-authorized application release may update the PM2 process after a canary build, smoke test, and rollback path are established.
...
- Release blockers requiring authorized external state: configure Google OAuth/Maps and email/payment variables where missing, create TEST role accounts in a disposable database, run authenticated role/E2E tests, and verify payment-sandbox behavior. The additive migration has now been applied to the authorized deployment database.
